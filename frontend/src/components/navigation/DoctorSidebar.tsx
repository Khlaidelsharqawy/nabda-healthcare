...
const registryLinks = [
  ...links.slice(0, 2),
  links[2],
  { href: '#', icon: 'stethoscope', message: 'workspaceShort' as const },
...
export function DoctorSidebar({ pathname, arabic, variant = 'default' }: { pathname: string; arabic: boolean; variant?: 'default' | 'registry' }) {
  ...
  return (
    <aside className="doctor-sidebar" aria-label={messages.workspace}>
      ...
      <nav className="doctor-sidebar__nav">
        {navigation.map((link, index) => {
          ...
          return (
            <a className={active ? 'doctor-sidebar__link is-active' : 'doctor-sidebar__link'} href={link.href} key={`${link.href}-${index}`}>
